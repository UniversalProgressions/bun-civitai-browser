...
const controller = new Elysia({ prefix: `/download` })
  ...
  .post(
    ...
    async ({ body }) => {
      ...
      const gopeedRes = await requester.get(
        `${settings.gopeed_api_host}/api/v1/info`,
      );
      if (!gopeedRes.ok) {
        throw new ExternalServiceError(
          `Failed to connect to gopeed server, please check your network connection, VPN tunnel and Gopeed service status.`,
        );
      }
      ...
      const modelVersion: ModelVersion = model.modelVersions.find(
      ...
      for (let index = 0; index < modelVersion.files.length; index++) {
        ...
        if (res.ok) {
          ...
          switch (res.status) {
            ...
            // Timeout
            default:
              throw new ResolveResourceUrlFailedError(
      ...
      for (let index = 0; index < modelFileDownloadTasks.length; index++) {
        ...
        if (await Bun.file(join(task.opts!.path!, task.opts!.name!)).exists()) {
          continue;
        }
      ...
      for (let index = 0; index < modelVersion.images.length; index++) {
        ...
        if (
          await Bun.file(
            join(mediaDir, extractFilenameFromUrl(media.url)),
          ).exists()
        ) {
          continue;
        }
